...
document.querySelectorAll('[data-choice]').forEach((select) => {
	...
	const choicesLocalization = {
		...
		noChoicesText: {
			en: 'No choices to chose form',
			ru: 'Вариантов для выбора больше нет',
		},
	};
 ...
 */
document.querySelectorAll('[data-switcher]').forEach(switcherElement => {
	...
	const checkSwitcherChange = (switcher) => {
		const switcherElems = switcher.closest('form').querySelectorAll(`[name="${switcher.name}"]`);
	...
	const switcherType = getSwitcherType(switcherElement);

	if (switcherType) { 
		switcherType === 'choice' ? initChoiceSwitcher(switcherElement) : initSwitcher(switcherElement);
	} else {
