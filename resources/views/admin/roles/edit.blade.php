						...
						<div class="form__row">
							<div class="form__column">
								<label class="form__label">
									...
									<input class="form__input input @error('name') _error @enderror" type="text" name="name" value="{{ $role->name }}" placeholder="Введите название роли">
								</label>
									...
									<div class="form__group">
										@foreach ($permissionGroup['permissions'] as $permission)
											<input type="checkbox" name="permissions[]" value="{{ $permission }}" data-check data-label="{{ __($permission) }}" 
												@if ($rolePermissions->contains($permission)) checked @endif>
										@endforeach
									</div>
								</div>
							@endforeach
